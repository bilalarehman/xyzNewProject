...
class Aauth {
	...
	public function __construct() {
		...
		$this->db_centralized = $this->CI->load->database('centralized', TRUE);
	...
	public function login($email, $pass, $remember = FALSE) {
		...
		*/
		...
		$query = null;
		$query = $this->db_centralized->where('email', $email);
		$query = $this->db_centralized->get($this->config_vars['users']);
		$row = $query->row();
		...
		$query = null;
		$query = $this->db_centralized->where('email', $email);
		$query = $this->db_centralized->where('banned', 1);
		$query = $this->db_centralized->where('verification_code !=', '');
		$query = $this->db_centralized->get($this->config_vars['users']);
		...
		$query = $this->db_centralized->where('pass', $this->hash_password($pass, $user_id));
		$query = $this->db_centralized->where('banned', 0);
		...
		// cookie control
			...
			if( ! $this->CI->input->cookie('user', TRUE) ){
				...
				if(!is_numeric( $cookie[0] ) OR strlen($cookie[1]) < 13 ){return FALSE;}
					...
					$query = $this->db_centralized->where('id', $cookie[0]);
					$query = $this->db_centralized->where('remember_exp', $cookie[1]);
					$query = $this->db_centralized->get($this->config_vars['users']);
	...
	public function login_fast($user_id){
		...
		$query = $this->db_centralized->where('id', $user_id);
		$query = $this->db_centralized->where('banned', 0);
		$query = $this->db_centralized->get($this->config_vars['users']);
	...
	public	function reset_login_attempts($user_id) {
		...
		$this->db_centralized->where('id', $user_id);
		return $this->db_centralized->update($this->config_vars['users'], $data);
	}
	...
	public function remind_password($email){
		...
		if ($query->num_rows() > 0){
			...
			$this->db_centralized->where('email', $email);
			$this->db_centralized->update($this->config_vars['users'], $data);
	...
	public function reset_password($user_id, $ver_code){
		...
		$query = $this->db_centralized->where('id', $user_id);
		$query = $this->db_centralized->where('verification_code', $ver_code);
		$query = $this->db_centralized->get( $this->config_vars['users'] );
	...
	public function create_user($email, $pass, $name) {
		...
		if ( $this->db_centralized->insert($this->config_vars['users'], $data )){

			$user_id = $this->db_centralized->insert_id();
	...
	public function list_users($group_par = FALSE, $limit = FALSE, $offset = FALSE, $include_banneds = FALSE) {
		...
		// banneds
		if (!$include_banneds) {
			$this->db_centralized->where('banned != ', 1);
		}
		...
		if ($limit) {
			...
			if ($offset == FALSE)
				$this->db_centralized->limit($limit);
			else
				$this->db_centralized->limit($limit, $offset);
		}

		$query = $this->db_centralized->get();
	...
	public function delete_user($user_id) {
		...
		$this->db_centralized->where('id', $user_id);
		$this->db_centralized->delete($this->config_vars['users']);

		// delete from perm_to_user
		$this->db_centralized->where('user_id', $user_id);
		$this->db_centralized->delete($this->config_vars['perm_to_user']);

		// delete from user_to_group
		$this->db_centralized->where('user_id', $user_id);
		$this->db_centralized->delete($this->config_vars['user_to_group']);

		// delete user vars
		$this->db_centralized->where('user_id', $user_id);
		$this->db_centralized->delete($this->config_vars['user_variables']);
	}
	...
	public function user_exsist_by_name( $name ) {
		$query = $this->db_centralized->where('name', $name);
	...
	public function user_exsist_by_email( $user_email ) {
		$query = $this->db_centralized->where('email', $user_email);
	...
	public function get_user_id($email=FALSE) {
		...
		if( ! $email){
			$query = $this->db_centralized->where('id', $this->CI->session->userdata('id'));
		} else {
			$query = $this->db_centralized->where('email', $email);
		}
	...
	public function get_user_groups($user_id = FALSE){
		...
		$this->db_centralized->select('*');
		$this->db_centralized->from($this->config_vars['user_to_group']);
		$this->db_centralized->join($this->config_vars['groups'], "id = group_id");
		$this->db_centralized->where('user_id', $user_id);

		return $query = $this->db_centralized->get()->result();
	}
	 ...
	 */
	public function create_group($group_name, $group_def) {

		$query = $this->db_centralized->get_where($this->config_vars['groups'], array('name' => $group_name));
		...
		if ($query->num_rows() < 1) {
			...
			$this->db_centralized->insert($this->config_vars['groups'], $data);
			return $this->db_centralized->insert_id();
		}
	 ...
	 */
	public function update_group($group_par, $group_name, $group_def) {

		$group_id = $this->get_group_id($group_par);

		$data['name'] = $group_name;
		$data['definition'] = $group_def;

		$this->db_centralized->where('id', $group_id);
		return $this->db_centralized->update($this->config_vars['groups'], $data);
	}
	...
	$this->db_centralized->where('id',$group_id);
	$query = $this->db_centralized->get($this->config_vars['groups']);
	if ($query->num_rows() == 0){
		return FALSE;
	}
		...
		$this->db_centralized->where('group_id', $group_id);
		$this->db_centralized->delete($this->config_vars['user_to_group']);

		$this->db_centralized->where('id', $group_id);
		return $this->db_centralized->delete($this->config_vars['groups']);
	}
	...
	public function add_member($user_id, $group_par) {
		...
		$query = $this->db_centralized->where('user_id',$user_id);
		$query = $this->db_centralized->where('group_id',$group_id);
		$query = $this->db_centralized->get($this->config_vars['user_to_group']);
		...
		if ($query->num_rows() < 1) {
			...
			return $this->db_centralized->insert($this->config_vars['user_to_group'], $data);
		}
	...
	public function remove_member($user_id, $group_par) {
		...
		$group_par = $this->get_group_id($group_par);
		$this->db_centralized->where('user_id', $user_id);
		$this->db_centralized->where('group_id', $group_par);
		return $this->db_centralized->delete($this->config_vars['user_to_group']);
	}
	...
	public function get_group_name($group_id) {
		...
		$query = $this->db_centralized->where('id', $group_id);
		$query = $this->db_centralized->get($this->config_vars['groups']);
	...
	public function get_group_id ( $group_par ) {
		...
		$query = $this->db_centralized->where('name', $group_par);
		$query = $this->db_centralized->get($this->config_vars['groups']);
	...
	public function create_perm($perm_name, $definition='') {
		...
		$query = $this->db_centralized->get_where($this->config_vars['perms'], array('name' => $perm_name));
		...
		if ($query->num_rows() < 1) {
			...
			$this->db_centralized->insert($this->config_vars['perms'], $data);
			return $this->db_centralized->insert_id();
		}
	...
	public function update_perm($perm_par, $perm_name=FALSE, $definition=FALSE) {
		...
		$this->db_centralized->where('id', $perm_id);
		return $this->db_centralized->update($this->config_vars['perms'], $data);
	}
	...
	public function delete_perm($perm_par) {
		...
		$this->db_centralized->where('perm_id', $perm_id);
		$this->db_centralized->delete($this->config_vars['perm_to_group']);
		...
		$this->db_centralized->where('id', $perm_id);
		return $this->db_centralized->delete($this->config_vars['perms']);
	}
	...
	public function is_allowed($perm_par, $user_id=FALSE){
		...
		$query = $this->db_centralized->where('perm_id', $perm_id);
		$query = $this->db_centralized->where('user_id', $user_id);
		$query = $this->db_centralized->get( $this->config_vars['perm_to_user'] );
	...
	public function is_group_allowed($perm_par, $group_par=FALSE){
		...
		if($group_par != FALSE){
			...
			$query = $this->db_centralized->where('perm_id', $perm_id);
			$query = $this->db_centralized->where('group_id', $group_par);
			$query = $this->db_centralized->get( $this->config_vars['perm_to_group'] );
	...
	public function allow_user($user_id, $perm_par) {
		...
		if ($query->num_rows() < 1) {
			...
			return $this->db_centralized->insert($this->config_vars['perm_to_user'], $data);
		}
		return TRUE;
	}
	...
	public function deny_user($user_id, $perm_par) {
		...
		return $this->db_centralized->delete($this->config_vars['perm_to_user']);
	}
	...
	public function allow_group($group_par, $perm_par) {
		...
		if ($query->num_rows() < 1) {
			...
			return $this->db_centralized->insert($this->config_vars['perm_to_group'], $data);
		}
	...
	public function deny_group($group_par, $perm_par) {
		...
		return $this->db_centralized->delete($this->config_vars['perm_to_group']);
	}
	...
	public function list_perms() {
		...
		$query = $this->db_centralized->get($this->config_vars['perms']);
		return $query->result();
	}
	...
	public function get_perm_id($perm_par) {
		...
		$query = $this->db_centralized->where('name', $perm_par);
		$query = $this->db_centralized->get($this->config_vars['perms']);
	...
	public function send_pm( $sender_id, $receiver_id, $title, $message ){
		...
		$query = $this->db_centralized->where('id', $receiver_id);
		$query = $this->db_centralized->where('banned', 0);
		...
		$query = $this->db_centralized->where('id', $sender_id);
		$query = $this->db_centralized->where('banned', 0);
		...
		return $query = $this->db_centralized->insert( $this->config_vars['pms'], $data );
	}
	...
	public function list_pms($limit=5, $offset=0, $receiver_id = FALSE, $sender_id=FALSE){
		...
		if ( $receiver_id != FALSE){
			$query = $this->db_centralized->where('receiver_id', $receiver_id);
		}

		if( $sender_id != FALSE ){
			$query = $this->db_centralized->where('sender_id', $sender_id);
		}

		$query = $this->db_centralized->order_by('id','DESC');
		$query = $this->db_centralized->get( $this->config_vars['pms'], $limit, $offset);
		return $query->result();
	}
	...
	public function get_pm($pm_id, $set_as_read = TRUE){
		...
		$query = $this->db_centralized->where('id', $pm_id);
		$query = $this->db_centralized->get( $this->config_vars['pms'] );
	...
	public function delete_pm($pm_id){
		...
		return $this->db_centralized->delete( $this->config_vars['pms'], array('id' => $pm_id) );
	}
	...
	public function count_unread_pms($receiver_id=FALSE){
		...
		$query = $this->db_centralized->where('receiver_id', $receiver_id);
		$query = $this->db_centralized->where('read', 0);
		$query = $this->db_centralized->get( $this->config_vars['pms'] );
	...
	public function set_as_read_pm($pm_id){
		...
		$this->db_centralized->update( $this->config_vars['pms'], $data, "id = $pm_id");
	}
	...
	public function set_user_var( $key, $value, $user_id = FALSE ) {
		...
		// if var not set, set
		 if ($this->get_user_var($key,$user_id) ===FALSE) {
			...
			return $this->db_centralized->insert( $this->config_vars['user_variables'] , $data);
		}
		// if var already set, overwrite
			...
			$this->db_centralized->where( 'key', $key );
			$this->db_centralized->where( 'user_id', $user_id);

			return $this->db_centralized->update( $this->config_vars['user_variables'], $data);
		}
	}
	...
	public function unset_user_var( $key, $user_id = FALSE ) {
		...
		return $this->db_centralized->delete( $this->config_vars['user_variables'] );
	}
	...
	public function get_user_var( $key, $user_id = FALSE){
		...
		$query = $this->db_centralized->where('user_id', $user_id);
		$query = $this->db_centralized->where('key', $key);

		$query = $this->db_centralized->get( $this->config_vars['user_variables'] );
	...
	public function list_user_var_keys($user_id = FALSE){
		...
		$query = $this->db_centralized->select('key');
	...
	public function set_system_var( $key, $value ) {
		...
		if ( ! $this->get_system_var($key) ) {
			...
			return $this->db_centralized->insert( $this->config_vars['system_variables'] , $data);
		...
		// if var already set, overwrite
			...
			$this->db_centralized->where( 'key', $key );
			return $this->db_centralized->update( $this->config_vars['system_variables'], $data);
		}
	...
	public function unset_system_var( $key	) {
		...
		return $this->db_centralized->delete( $this->config_vars['system_variables'] );
	}
	...
	public function get_system_var( $key ){
		...
		$query = $this->db_centralized->get( $this->config_vars['system_variables'] );
